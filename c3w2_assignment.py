...
Original file is located at
    https://colab.research.google.com/github/https-deeplearning-ai/tensorflow-3-public/blob/main/Course%203%20-%20Advance%20Computer%20Vision/W2/assignment/C3W2_Assignment.ipynb
...
The packages required for this lab requires Python 3.11 which is not what the current Colab runtime uses. [Please follow the instructions here to use the fallback](https://community.deeplearning.ai/t/tf-at-using-the-fallback-runtime/530596). Otherwise, you might run into import errors and other issues. The test below will check if Colab is using the expected runtime.
"""
...
- Important: Be careful to avoid typos in the key names for the checkpoint. For example, if there is a layer called `_apple_pies` and you accidentally added an extra "t" like this:
`tmp_pies_checkpoint = tf.train.Checkpoint( _apple_piest = detection_model._box_predictor._apple_pies )`
then, when you restore the checkpoint, it will update the variable `_apple_piest`, instead of `_apple_pies` like you intended. This will likely make the model train slower in Exercise 10 later.
"""
...
for var in detection_model.trainable_variables:
    if ('WeightSharedConvolutionalBoxPredictor/WeightSharedConvolutionalBoxHead' in var.name or
        'WeightSharedConvolutionalBoxPredictor/WeightSharedConvolutionalClassHead' in var.name):
...
def train_step_fn(image_list,
    ...
    ### START CODE HERE (Replace instances of `None` with your code) ###
        ...
        for image in image_list:
            processed_img, true_shape = model.preprocess(image)
            preprocessed_image_list.append(processed_img)
            true_shape_list.append(true_shape)
